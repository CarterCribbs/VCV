...
csv_NYISOC_file_name = str_start_date + "-" + str_end_date + " NYISO Day-Ahead Energy Price.csv"

'''
Here we read the CSV of hourly locational marginal pricing (LMP) from different energy HUBS in NYISO's power grid
'''

lmp = pd.read_csv("/Users/cartercribbs/Downloads/" + csv_NYISOC_file_name)
lmp = lmp.loc[lmp['Zone']=='CENTRL']
...
'''
Here we are working with LMP data soley from Centrl Hub
...
lmp_chart = lmp_dayMean.plot('Date', 'LBMP', color = "k", linestyle = '-', linewidth = 1.8)
ax = plt.gca()
...
plt.title("Daily Average LMP Rate of Centrl Hub from NYISO Over One Year")

plt.savefig("Central Hub Data.png")
